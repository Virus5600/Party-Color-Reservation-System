...
import { faPerson, faEnvelope, faPhone, faCalendar, faClock, faCircleCheck, faClipboard, faCircleXmark } from '@fortawesome/free-solid-svg-icons';
...
// under development
const ReservationComfirmed = ({ onClickReturn, number, date, time }) => {
	return (
		<div className='ReservationConfirmed'>
			...
			<div className='ReservationConfirmed-details'>
				...
				<div className='ReservationConfirmed-details-button'>
					<button onClick={onClickReturn} >Close</button>
				</div>
...
const ReservationForm = ({ onClickReturn }) => {
	...
	if (success) {
		return <ReservationComfirmed onClickReturn={onClickReturn} number={number} date={date} time={time} />;
	} else {
		return (
			<div>
