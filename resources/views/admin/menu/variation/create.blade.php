...
@section('title', "Menu Variation - {$menu->name} (Add Variation)")
	...
	<div class="row">
		<div class="mx-auto col-12 col-lg-10">
			<div class="card dark-shadow mb-5" id="inner-content">
				<div class="card-body">
					<form action="{{ route('admin.menu.variation.store', [$menu->id]) }}" method="POST" enctype="multipart/form-data">
						{{ csrf_field() }}
						...
						<div class="row">
							...
							<div class="form-group col-12 col-lg-4">
								...
								<input class="form-control" type="text" name="variation_name" id="variation_name" value="{{ old('variation_name') }}"/>
							...
							<div class="form-group col-12">
								...
								<div class="row" id="itemField">
									@php ($index = 0)
									@if (old('variation_item') || old('amount'))
										{{-- Iterate through the old menu item values --}}
										@foreach (old('variation_item') as $mi)
												...
												<div class="card-body">
													...
													<div class="form-group">
														...
														<select class="show-tick select-picker w-100" name="variation_item[]">
															<option data-hidden="true" value="0" {{ old("variation_item.{$index}") ? '' : 'selected' }}>Select</option>
															@foreach ($items as $i)
															<option value="{{ $i->id }}" data-subtext="{{ $i->measurement_unit }}" {{ old("variation_item.{$index}") == $i->id ? 'selected' : '' }}>{{ $i->item_name }}</option>
															@endforeach
														</select>

														<br><span class="text-danger validation">{{ $errors->first("variation_item.".old("new_index")[$index]) }}</span>
													</div>
													...
													<div class="form-group">
														...
														<div class="input-group">
															...
															<div class="input-group-append">
																<span class="input-group-text unit-of-measurement">{{ old("variation_item.{$index}") ? $items->where('id', '=', old("variation_item.{$index}"))->first()->measurement_unit : "kg"}}</span>
										...
										<div class="card h-100">
											<div class="card-body">
												...
												<div class="form-group">
													...
													<br><span class="text-danger">{{ $errors->first('variation_item.0') }}</span>
												</div>
