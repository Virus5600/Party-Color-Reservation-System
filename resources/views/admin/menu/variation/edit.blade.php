...
@section('title', "Menu Variation - {$menu->name} (Edit Variation - {$variation->name})")
	...
	<div class="row">
		<div class="col-12 col-md mt-3">
			<div class="row">
				...
				<div class="col-12">
					<h1>
						<a href="javascript:void(0);" onclick="confirmLeave('{{route('admin.menu.variation.index', [$menu->id])}}');" class="text-dark text-decoration-none font-weight-normal">
	...
	<div class="row">
		<div class="mx-auto col-12 col-lg-10">
			<div class="card dark-shadow mb-5" id="inner-content">
				<div class="card-body">
					<form action="{{ route('admin.menu.variation.update', [$menu->id, $variation->id]) }}" method="POST" enctype="multipart/form-data">
						{{ csrf_field() }}

						<div class="row">
							{{-- MENU NAME --}}
							<div class="form-group col-12 col-lg-4">
								<label class="h5" for="variation_name">Variation Name</label>
								<input class="form-control" type="text" name="variation_name" id="variation_name" value="{{ $variation->name }}"/>
								<span class="text-danger">{{ $errors->first('variation_name') }}</span>
							</div>
							...
							<div class="form-group col-12 col-lg-4">
								...
								<div class="input-group">
									...
									<input type="number" class="form-control" min="0" max="4294967295" step=".25" name="price" value="{{ number_format($variation->price, 2, ".", "") }}">
								</div>
							...
							<div class="form-group col-12 col-lg-4">
								...
								<div class="input-group">
									...
									<input class="form-control has-spinner text-right" type="number" name="duration_hour" id="duration_hour" min="0" max="12" value="{{ $variation->getFromDuration("H") }}"/>
									
									<div class="input-group-prepend input-group-append">
										<span class="input-group-text">:</span>
									</div>
									
									<input class="form-control has-spinner" type="number" name="duration_min" id="duration_min" min="0" max="59" value="{{ $variation->getFromDuration("i") }}"/>
							...
							<div class="form-group col-12">
								...
								<div class="row" id="itemField">
									...
									@forelse ($variation->items as $vi)
									<div class="col-12 col-md-4 my-2 position-relative" {{ $index == 0 ? 'id=origForm' : ''}}>
										<div class="card h-100">
											<div class="card-body">
												{{-- ITEM NAME --}}
												<div class="form-group">
													<label class="form-label" for="variation_item[]">Item Name</label><br>
													
													<select class="show-tick select-picker w-100" name="variation_item[]">
														<option data-hidden="true" data-default value="0" selected>Select</option>
														@foreach ($items as $i)
														<option value="{{ $i->id }}" data-subtext="{{ $i->measurement_unit }}" {{ $vi->id == $i->id ? 'selected' : '' }}>{{ $i->item_name }}</option>
														@endforeach
													</select>
													
													<br><span class="text-danger validation">{{ $errors->first("variation_item.{$index}") }}</span>
												</div>
												...
												<div class="form-group">
													...
													<div class="input-group">
														...
														<input type="number" class="form-control" min="0" max="4294967295" step="0.01" name="amount[]" value="{{ $vi->pivot->amount }}" />
														<div class="input-group-append">
															<span class="input-group-text unit-of-measurement">{{ $vi->measurement_unit }}</span>
										...
										<div class="card h-100">
											<div class="card-body">
												...
												<div class="form-group">
													...
													<br><span class="text-danger validation">{{ $errors->first('variation_item.0') }}</span>
												</div>
						...
						<div class="d-flex">
							...
							<a href="javascript:void(0);" onclick="confirmLeave('{{route('admin.menu.variation.index', [$menu->id])}}');" class="btn btn-danger ml-3 mr-auto">Cancel</a>
						</div>
					</form>
				</div>
			</div>
...
<script type="text/javascript">
	$(document).ready(() => {
		...
		$('[name="amount[]"]:not([readonly])').trigger('change');
