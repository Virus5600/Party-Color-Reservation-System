		...
		<div class="col-12 col-md-11 col-lg-10 mx-auto">
			<div class="card dark-shadow mb-5" id="inner-content">
				<h4 class="card-header">
					Manage Permissions for "{{ $type->name }}" User Type
				</h4>

				<div class="card-body">
					<form method="POST" action="{{ route('admin.types.update-permissions', [$type->id]) }}" enctype="multipart/form-data" class="form needs-validation" novalidate>
						{{ csrf_field() }}
									...
									<div class="card">
										<div class="card-body">
											...
											<div class="form-check col-12">
												<input type="checkbox" name="permissions[]" id="perms_{{ $p->slug }}" {{ in_array($p->id, $permIds) ? 'checked' : '' }} value="{{ $p->id }}" {{ ($pp = $p->parentPermission()) != null ? "data-parent=#perms_{$pp->slug}" : "" }}>
											...
											@foreach($p->childPermissions() as $cp)
												...
												<div class="form-check col-12 ml-4">
													<input type="checkbox" name="permissions[]" id="perms_{{ $cp->slug }}" {{ in_array($p->id, $permIds) ? 'checked' : '' }} value="{{ $cp->id }}" data-parent="#perms_{{ $p->slug }}">
