		...
		eventClick: function (e, el) {
			...
			$.get(showBooking.replace('%241', data.data_id), (response) => {
				if (response.success) {
					...
					let cancelReqButtons = `<button onclick="updateCalendar(${booking.id}, '${acceptCancelBooking.replace('%241', booking.id)}', undefined, true, 'Are you sure you want to approve the cancellation of this booking?')" class="btn btn-secondary"><i class="fas fa-circle-check mr-0 mr-sm-2"></i><span class="d-none d-sm-inline-block">Approve</span></button>
						<button class="btn btn-secondary" data-scf="Reason" data-scf-name="reason" data-scf-custom-title="Reason for rejection" data-scf-target-uri="${rejectCancelBooking.replace('%241', booking.id)}" data-scf-use-textarea='true' data-scf-callback='${callback}'><i class="fas fa-circle-xmark mr-0 mr-sm-2"></i><span class="d-none d-sm-inline-block">Reject</span></a>`;
